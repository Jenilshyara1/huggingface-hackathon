...
|------------------------------------|---------------------|-------:|-------:|-------:|-----:|--------:|
| `llama-3.3-70b-versatile`          | Groq                | **1.000** | **0.857** | **1.000** | **0.952** | 296 s |
| `Qwen/Qwen2.5-7B-Instruct`         | HuggingFace Router  | 1.000 | — | — | — | 3.4 s (Task 1 only) |
| *scripted optimal reference*       | deterministic agent | 1.000 | 1.000 | 1.000 | 1.000 | — |

### What the scores mean

- **Task 1 (easy) — 1.000**: Llama 3.3 70B solved the duplicate-refund task in 4 steps (`search_customers` → `list_invoices` → `create_refund` → `submit`). Expected baseline for a capable instruction-following model.
- **Task 2 (medium) — 0.857**: The F1 of 0.857 decomposes as **precision 1.00, recall 0.75** — the model correctly discounted 3 of the 4 qualifying customers with **zero wrong discounts**, missing one. This is a real-world failure mode (skipping a borderline qualifier) rather than a guess-and-check failure, which is exactly the kind of signal this environment is designed to surface.
...
The `Qwen/Qwen2.5-7B-Instruct` row validates that the pipeline is provider-agnostic (Task 1 solved identically via HuggingFace's inference router). The scripted-reference row is not an LLM score — it's generated by invoking the same `step()` code path a perfect agent would take, establishing the achievable upper bound and confirming all three graders return `1.0` on correct trajectories.
...
```bash
# From project root
export API_BASE_URL="https://api.openai.com/v1"       # any OpenAI-compatible endpoint
export MODEL_NAME="gpt-4o-mini"                        # or llama-3.3-70b-versatile via Groq, Qwen2.5-7B-Instruct via HF router, etc.
export HF_TOKEN="sk-..."                               # your API key (OpenAI/HF/Groq/etc.)
...
```bash
cd finops_env
docker build -t finops-env:latest -f server/Dockerfile .
...
```bash
openenv validate ./finops_env
# [OK] finops: Ready for multi-mode deployment
```
...
├── tasks.py                 # TaskConfig dataclasses wiring tickets → seeds → graders
├── graders.py               # Deterministic task graders (F1, weighted F1)
└── server/
    ├── app.py               # FastAPI app via openenv.core.http_server
    ├── finops_env_environment.py  # FinopsEnvironment(Environment) — reset/step/state
    ├── Dockerfile
    ├── requirements.txt
    └── __init__.py

inference.py                 # Baseline LLM agent loop (project root, hackathon-required)
```
